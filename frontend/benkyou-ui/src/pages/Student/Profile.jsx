...
export default function StudentProfile() {
  ...
  const [profileForm, setProfileForm] = useState({ firstName: "", lastName: "", yearEnrolled: "", yearLevel: "", program: "" });
  ...
  const handleEditToggle = () => {
    if (!editMode) {
      ...
      setProfileForm({ 
        firstName: first || "", 
        lastName: rest.join(" ") || "",
        yearEnrolled: authUser?.yearEnrolled || "",
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await updateProfile({
        ...profileForm,
        yearEnrolled: profileForm.yearEnrolled ? parseInt(profileForm.yearEnrolled) : null,
        yearLevel: profileForm.yearLevel || null,
  ...
  if (isLoading) return (
    <DashboardLayout theme={STU_THEME} role="Student" navGroups={getNav("My Profile")} headerTitle="Profile" headerSub="Loading...">
      <div style={{ textAlign: "center", padding: 50, color: "#5a7a9a" }}>Loading profile...</div>
    </DashboardLayout>
  );

  return (
    <DashboardLayout theme={STU_THEME} role="Student" navGroups={getNav("My Profile")} userName={authUser?.name} headerTitle="Profile" headerSub="Manage your student profile">
      <div style={s.twoCol}>
        {/* Personal Info */}
        <div style={s.tableCard}>
          ...
          {editMode ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={fi.group}>
                <label style={fi.label}>YEAR ENROLLED</label>
                <input style={fi.input} type="number" value={profileForm.yearEnrolled} onChange={e => setProfileForm({...profileForm, yearEnrolled: e.target.value})} />
              </div>
              <div style={fi.group}>
                <label style={fi.label}>YEAR LEVEL</label>
                <select style={fi.input} value={profileForm.yearLevel} onChange={e => setProfileForm({...profileForm, yearLevel: e.target.value})}>
              ...
              <div style={fi.group}>
                <label style={fi.label}>PROGRAM</label>
                <input style={fi.input} type="text" placeholder="e.g. BSIT, BSCS, BSED" value={profileForm.program} onChange={e => setProfileForm({...profileForm, program: e.target.value})} />
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              ...
              <Field label="Year Enrolled" value={authUser?.yearEnrolled || "N/A"} />
              <Field label="Year Level" value={authUser?.yearLevel || "N/A"} />
              <Field label="Program" value={authUser?.program || "N/A"} />
              <Field label="Role" value={authUser?.role || "Student"} />
        ...
        {/* Academic Summary */}
        <div style={s.tableCard}>
          <h2 style={s.cardTitle}>Academic summary</h2>
          <div style={{ marginTop: 24, display: "flex", flexDirection: "column", gap: 16 }}>
            {[
              { label: "Enrolled courses", value: data?.stats?.totalCourses?.toString() || "0", icon: "📚", bg: "rgba(16,185,129,0.1)" },
              { label: "Active learning", value: data?.stats?.activeEnrollments?.toString() || "0", icon: "✅", bg: "rgba(59,130,246,0.1)" },
              { label: "Completed", value: data?.stats?.completedEnrollments?.toString() || "0", icon: "🎓", bg: "rgba(245,158,11,0.1)" },
              { label: "Assessments", value: data?.stats?.totalAssessments?.toString() || "0", icon: "📝", bg: "rgba(139,92,246,0.1)" },
            ].map((item) => (
