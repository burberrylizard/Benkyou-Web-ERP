/**
 * Shared Instructor theme + nav builder.
 * Dark navy theme matching the Benkyou LMS design system.
 */
export const INST_THEME = {
  sidebar: "#0b1929",
  primary: "#185fa5",
  accent: "#5b9bd5",
  navLabel: "#4a7aa8",
  header: "#0f2035",
};

const ALL_ITEMS = [
  { group: "Teaching", label: "Dashboard", path: "/instructor/dashboard" },
  { group: "Teaching", label: "My Courses", path: "/instructor/courses" },
  { group: "Teaching", label: "Assessments", path: "/instructor/quizzes" },
  { group: "Students", label: "Class Progress", path: "/instructor/analytics" },
  { group: "Students", label: "Grade Book", path: "/instructor/grades" },
  { group: "Students", label: "Submissions", path: "/instructor/logs" },
  { group: "Account", label: "My Profile", path: "/instructor/settings" },
];

export function getNav(activeLabel) {
  const groups = {};
  ALL_ITEMS.forEach((item) => {
    if (!groups[item.group]) groups[item.group] = [];
    groups[item.group].push({ label: item.label, path: item.path, active: item.label === activeLabel });
  });
  return Object.entries(groups).map(([label, items]) => ({ label, items }));
}

/* ── Dark Theme Styles ── */
export const INST_STYLES = {
  statsGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 24 },
  card: { background: "#111c2e", border: "1px solid #1e3a5f", borderRadius: 14, padding: 24 },
  statLabel: { fontSize: 12, color: "#7a8ba3", fontWeight: 500, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.03em" },
  statValue: { fontSize: 32, fontWeight: 700, color: "#e8edf4", marginBottom: 4, letterSpacing: "-0.02em" },
  tableCard: { background: "#111c2e", border: "1px solid #1e3a5f", borderRadius: 14, padding: 24, marginBottom: 24 },
  cardHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 },
  cardTitle: { fontSize: 17, fontWeight: 600, color: "#e8edf4" },
  primaryBtn: { background: "#185fa5", color: "#fff", border: "none", padding: "8px 18px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", transition: "all 0.2s" },
  outlineBtn: { background: "transparent", color: "#8ea4bd", border: "1px solid #2a4060", padding: "8px 18px", borderRadius: 8, fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.2s" },
  table: { width: "100%", borderCollapse: "collapse", textAlign: "left" },
  th: { fontSize: 11, color: "#5a7a9a", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", paddingBottom: 14, borderBottom: "1px solid #1e3a5f" },
  tr: { borderBottom: "1px solid rgba(30,58,95,0.5)" },
  td: { padding: "14px 0", fontSize: 14, color: "#a0b4cb" },
  twoCol: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(400px, 1fr))", gap: 20, marginBottom: 24 },
  progressBg: { height: 6, background: "#1a2d45", borderRadius: 3, overflow: "hidden" },
  progressFill: { height: "100%", background: "#185fa5", borderRadius: 3, transition: "width 0.5s ease" },
  tinyAvatar: { width: 36, height: 36, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 600, color: "#fff", flexShrink: 0 },
  actionBtn: { background: "transparent", border: "1px solid #2a4060", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 14, color: "#7a8ba3" },
  /* Course card styles */
  courseCard: { background: "#111c2e", border: "1px solid #1e3a5f", borderRadius: 14, overflow: "hidden", transition: "border-color 0.2s" },
  courseCardInner: { padding: 20 },
  courseCardAccent: { height: 3 },
  badge: (bg, color, border) => ({ background: bg, color, border: `1px solid ${border}`, padding: "3px 10px", borderRadius: 10, fontSize: 11, fontWeight: 600 }),
};
